...
int main(int argc, char** argv) {
  ...
  while (ros::ok()) {
    ...
    // std::cout << 10 * rand() / (RAND_MAX + 1.0f) << std::endl;
    // std::cout << (rand() % (255 + 1)) << std::endl;
    for (int i = 0; i < num_points; i++)
    {
        ...
        cloud.points[i].r = 111; //(rand() % (b-a+1))+ a
        cloud.points[i].g = 222;
        cloud.points[i].b = 123;
    }
    pcl::toROSMsg(cloud, output_msg);  //将点云转化为消息才能发布
    output_msg.header.frame_id = "camera_link";  //frame_id为camera_link，rviz中就不用改了
